...
export default function Notes() {
	...
	const columnCount: number =
		possibleColumnCount < 2 ? 2 : possibleColumnCount;
	...
	return (
		<View className="flex flex-1 m-4 rounded-xl overflow-hidden ">
			<Text className="text-text text-3xl my-4">Notes</Text>
			<View className="flex-1 rounded-xl overflow-hidden borer-2 border-primary">
				<MasonryList
					data={notesArray ?? []}
					renderItem={({ item }) => (
						<NoteCard note={item as NoteWithUUID} />
					)}
...
const NoteCard = ({ note }: { note: NoteWithUUID }) => {
	...
	return (
		...
		>
			...
			>
				<View className="">
					<Text className="text-xl text-text">{note?.title}</Text>
				</View>
				<View className="">
