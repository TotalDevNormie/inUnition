...
export const useSearchStore = create<SearchState>((set, get) => ({
  ...
  search: () => {
    const { searchTerm, selectedTypes, selectedTags, dueDateFilter, sortBy, sortDirection } = get();
    ...
    if (selectedTags.length > 0) {
      results = results.filter(
        (item) => item.tags && selectedTags.some((tag) => item.tags?.includes(tag))
      );
    }
  ...
  getAllTags: () => {
    const noteTags = useNoteStore
      .getState()
      .activeNotesArray()
      .flatMap((note) => note.tags || []);

    const taskTags = useTaskStore
      .getState()
      .activeTasksArray()
      .flatMap((task) => task.tags || []);

    const taskBoardTags = useTaskBoardStore
      .getState()
      .activeTaskBoards()
