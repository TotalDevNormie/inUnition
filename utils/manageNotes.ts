...
import { setDoc, deleteDoc, fetchAll, listen } from '../firestoreAdapter';
  ...
  persist(
    (set, get) => {
      const debouncedFirebaseSync = debounce(async (updatedNote: Note) => {
        try {
          const uuid = updatedNote.uuid || uuidv4();
          const user = useAuthStore.getState().user!;
          await setDoc(CST, uuid, { ...updatedNote, userUid: user.uid });
      ...
      return {
        ...
        pending: {},
        lastSync: Date.now(),
        activeNotesArray: () => Object.values(get().notes).filter((n) => n.state === 'active'),
        notesWithTag: (tag) => Object.values(get().notes).filter((n) => n.tags?.includes(tag)),

        saveNote: async (note) => {
          const uuid = note.uuid || uuidv4();
          const nowIso = new Date().toISOString();
          const old = get().notes[uuid] || {};
          const title = note.title !== undefined ? note.title.slice(0, 100) : old.title || '';
          const content =
            note.content !== undefined ? note.content.slice(0, 10000) : old.content || '';
          const updated: Note = {
            ...old,
            ...note,
            uuid,
            title,
            titleUpdatedAt: note.title !== undefined ? nowIso : old.titleUpdatedAt || nowIso,
            content,
            contentUpdatedAt: note.content !== undefined ? nowIso : old.contentUpdatedAt || nowIso,
            state: 'active',
          ...
          if (info.isConnected && useAuthStore.getState().isAuthenticated) {
            debouncedFirebaseSync(updated);
          }
        },
        ...
        syncWithFirebase: async () => {
          ...
          for (const [id, r] of Object.entries(remote)) {
            ...
            m.state = l.state === 'deleted' || r.state === 'deleted' ? 'deleted' : 'active';
            merged[id] = m;
          }
