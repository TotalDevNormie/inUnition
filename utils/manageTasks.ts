...
export const deleteTaskGroup = async (
  ...
  if (state.isConnected && user && !withoutDatabase) {
    ...
    const oldBuffer = await AsyncStorage.getItem(`deletedTaskGroups-buffer`);
    if (oldBuffer) {
      const buffer = JSON.parse(oldBuffer);
      buffer.push(uuid);
      await AsyncStorage.setItem(
...
export async function getTasks(
  ...
  console.log("from here", skipDBSync, taskGroupUUID, getFromBuffer, tasks);

  //   if (!getFromBuffer && state.isConnected && user && !skipDBSync)
  //     getTasksFromDB();

  if (taskGroupUUID && !getFromBuffer) {
    console.log("from here", tasks);
    console.log(tasks.filter((task) => task.taskGroupUUID === taskGroupUUID));
...
export const saveTask = async (
  ...
  const newStatus: string =
    task.status && taskGroup.statusType.includes(task.status)
      ? task.status
      : (oldTask?.status as string) || taskGroup.statusType[0];
  ...
  console.log("from 477");
  const tasks = await getTasks(false, saveToBuffer);
  console.log("from 479");

  let newTasks: Task[] = [];
  if (oldTask) {
    console.log("from 483");
    ...
    console.log("from 490");
    newTasks = tasks;
    console.log("from 492", newTasks);
    newTasks.push(newTask);
    console.log("from 493");
  }
  console.log("from 491");
...
export const deleteTask = async (taskUUID: string): Promise<void> => {
  ...
  if (oldDeletedEntries) {
    ...
    await AsyncStorage.setItem("deletedTaskEntries", JSON.stringify(deleted));
  } else {
    await AsyncStorage.setItem(
      "deletedTaskEntries",
